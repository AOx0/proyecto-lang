...
static const yytype_uint16 yyrline[] = {
    0,   197, 197, 197, 307, 312, 319, 321, 323, 325, 339, 356, 373, 393, 394,
    398, 399, 400, 401, 404, 404, 406, 409, 409, 420, 420, 433, 446, 449, 459,
    475, 476, 476, 477, 477, 479, 480, 480, 480, 481, 481, 481, 483, 484, 485,
    486, 487, 488, 491, 494, 495, 496, 497, 498, 502, 506, 507, 508, 511, 515,
    516, 519, 520, 520, 521, 522, 523, 546, 601, 601, 602, 602, 603, 603, 604,
    604, 605, 606, 606, 606, 606, 606, 606, 607, 607, 610, 623, 626, 632, 634,
    667, 669, 702, 761, 780, 812, 836, 853, 870, 892};
#endif
...
yyreduce:
...

    {
        ...
        (yyvsp[(3) - (11)].symbol).info.fun =
            (FunctionInfo){.args = (yyvsp[(5) - (11)].idents),
    ...
    {
        Symbol *s = (Symbol *)assert_sym_exists(&(yyvsp[(1) - (1)].symbol));
        if (s->type != Procedure) {
            str_clear(&wrn_buff);
        ...
        (yyval.function_call) =
            (FunctionCall){.symbol = (yyvsp[(1) - (1)].symbol), .args = t};
    } break;

    case 67:

    {
        Symbol *s = (Symbol *)assert_sym_exists(&(yyvsp[(1) - (4)].symbol));
        ...
        printf("Llamado a funcion: %.*s que devuelve %s\n",
               (int)(yyvsp[(1) - (4)].symbol).name.len,
               (yyvsp[(1) - (4)].symbol).name.ptr,
               data_type_e_display_return(&s->info.fun.return_type.type));
...

    {
        ...
        Node *past_root =
            (Node *)vec_get(&(yyvsp[(1) - (3)].subtree).values, 0);
        Node *curr_root =
            (Node *)vec_get(&(yyvsp[(3) - (3)].subtree).values, 0);

        if (past_root->asoc_type != curr_root->asoc_type) {
            str_clear(&wrn_buff);
            str_push(
                &wrn_buff,
                "Error: Se intento sumar dos expresiones de tipos distintos: ");
            str_push(&wrn_buff, "El primer operando es de tipo ");
            str_push(&wrn_buff,
                     data_type_e_display_return(&past_root->asoc_type));
            str_push(&wrn_buff, " y el segundo es de tipo ");
            str_push(&wrn_buff,
                     data_type_e_display_return(&curr_root->asoc_type));
            yyerror(str_as_ref(&wrn_buff));
        }
...

    {
        ...
        *n = (Node){
            .node_type = NExpr,
            .value.expr =
                (ExprNode){.type = ESymbol,
                           .value.symbol = (yyvsp[(1) - (1)].symbol),
                           .asoc_type =
                               (yyvsp[(1) - (1)].symbol).info.var.type.type},
            .asoc_type = (yyvsp[(1) - (1)].symbol).info.var.type.type};
...

    {
        ...
        Node *past_root =
            (Node *)vec_get(&(yyvsp[(2) - (2)].subtree).values, 0);
